    ...
    def run(self):
        while True:
            ...
            for pos in self.snake_pos:
                pygame.draw.rect(self.win, (0, 255, 0), pygame.Rect(pos[0]*self.snake_size, pos[1]*self.snake_size, self.snake_size, self.snake_size))

            # Отрисовка яблока
            pygame.draw.rect(self.win, (255, 0, 0), pygame.Rect(self.apple_pos[0]*self.snake_size, self.apple_pos[1]*self.snake_size, self.snake_size, self.snake_size))

            # Отрисовка преграды
            for i in range(self.matrix_size):
                for j in range(self.matrix_size):
                    if self.matrix[i][j] == 3:
                        pygame.draw.rect(self.win, (0, 0, 255), pygame.Rect(i*self.snake_size, j*self.snake_size, self.snake_size, self.snake_size))
